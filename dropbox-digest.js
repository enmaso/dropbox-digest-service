...
console.log('Starting Dropbox Digest Service')
app.start()

function processFile (message, done) {
  let fileId = message.Body
  console.log('Downloading file', fileId)
  Document.findById(fileId)
  .then(file => {
      ...
      if (service === null) {
        ...
        let opts = {
          ...
          headers: {
            ...
            'Dropbox-API-Arg': `{'path': '${file.source.path_lower}'}`
          },
          encoding: null
        }
        request(opts, function (err, response, data) {
          if (err) {
            ...
            console.error(`Google Drive response ${response.statusCode} ${response.statusMessage}`)
